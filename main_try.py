...
def split_lesson(lesson_text):#splits the received string and returns a list with separate values
    if(lesson_text!=None):
...
#3
def check_place(schedule, lesson, number_of_hours):#Checks if the lesson can be entered without a problem and returns a boolean value.
...
#3
def insert_requested_lessons(schedule, lessons, number_of_hours):#Schedules lessons according to user input and possible conditions, and returns those that do not meet these requirements to the failed list.
    failed_lessons = []
...
#4
def try_again(schedule, lesson, number_of_hours, number_of_days):#Attempt to reschedule the lesson first on the original day and then on other days
    lesson_name = lesson[0]
    lesson_duration = lesson[1]
    lesson_day = lesson[2]
    lesson_start = lesson[3]
...
#4
def reinsert_failed_lessons(schedule, failed_lessons, number_of_hours, number_of_days):#Trying to reassign failed classes Returns a list of classes that have not yet been entered
    still_failed = []
...
def main():
    ...
    while(number_of_hours>LAST_HOUR-FIRST_HOUR):
        print("invalid input")
        number_of_hours = int(input("Enter number of study hours per day (max 8): "))
    ...
    if len(still_failed) == 0:
        print("All lessons inserted successfully after reinsertion if needed.")
        print_schedule(schedule)
    else:
