...
def put_lesson(schedule, lesson,number_of_hours):
    """
    מכניסה שיעור למערכת
    """
    lesson_name = lesson[0]
    ...
    day_index = DAYS_INDEX[lesson_day] - 1
    start_index = lesson_start - FIRST_HOUR
    if(lesson_duration>number_of_hours):
        schedule=over_hours(lesson, number_of_hours,schedule)
...
def try_again(schedule, lesson, number_of_hours, number_of_days):
    ...
    # ניסיון ביום המקורי
    for hour in range(FIRST_HOUR, LAST_HOUR - lesson_duration + 1,number_of_hours):
        trial_lesson = [lesson_name, lesson_duration, lesson_day, hour]
        if check_place(schedule, trial_lesson, number_of_hours):
            put_lesson(schedule, trial_lesson,number_of_hours)
...
def over_hours(lesson, number_of_hours,schedule):
    lesson_name = lesson[0]
    lesson_duration=lesson[1]
    lesson_day=lesson[2]
    lesson_start=lesson[3]
    ...
    for j in range(start_index,start_index + lesson_duration):
        schedule[day_index][j] = lesson_name
    return schedule
...
def insert_requested_lessons(schedule, lessons, number_of_hours):
    ...
    for lesson in lessons:
        if check_place(schedule, lesson, number_of_hours):
            put_lesson(schedule, lesson,number_of_hours)
        else:
